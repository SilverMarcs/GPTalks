...
struct BasicMarkdownView: View {
    ...
    init(text: String, highlightText: String) {
        self.contentItems = BasicMarkdownView.parseMarkdown(text)
        BasicMarkdownView.applyHighlighting(to: &self.contentItems, highlightText: highlightText)
    }
    ...
    private static func applyHighlighting(to contentItems: inout [ContentItem], highlightText: String) {
        for index in contentItems.indices {
            ...
            case .codeBlock(let codeString, let language):
                // No highlighting applied to code blocks
                contentItems[index] = .codeBlock(codeString, language: language)
            }
        }
    }
